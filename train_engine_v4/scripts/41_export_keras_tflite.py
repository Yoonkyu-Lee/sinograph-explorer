"""SCER Keras → INT8 TFLite (Edge TPU compatible) — Phase 3-2 (doc/29 §7).

Same path as Lab2 cell 23 / Phase 1 (`v3_keras_char_int8.tflite`):
    Keras (5 outputs)
      ↓ tf.lite.TFLiteConverter.from_keras_model()
      ↓ + representative_dataset (NHWC, [-1, 1])
    ...
    INT8 TFLite (Edge TPU compatible)
      ↓ edgetpu_compiler scer_int8.tflite -o deploy_pi/export/

Note: Keras model has Lambda layer (L2 normalize) → load with safe_mode=False.

Run from lab2-style WSL venv (Python 3.11, TF 2.15.0).

Usage:
    python train_engine_v4/scripts/41_export_keras_tflite.py \\
        --keras deploy_pi/export/scer_keras.keras \\
        --shard-dir synth_engine_v3/out/94_production_102k_x200 \\
        --out   deploy_pi/export/scer_int8.tflite \\
...
def collect_calib_nhwc(shard_dir: Path, n: int, input_size: int,
    ...
    """Pull N samples → (N, H, H, 3) float32 in [-1, 1] (NHWC)."""
    ...
    print(f"[calib] collected {out.shape} "
          f"range=[{out.min():.3f},{out.max():.3f}] (NHWC)")
    return out


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--keras", required=True, help="path to scer_keras.keras")
    ...
    print(f"[tf] loading Keras SCER (safe_mode=False for Lambda layer)...")
    keras_model = tf.keras.models.load_model(
        str(keras_path), compile=False, safe_mode=False,
    )
    print(f"[tf] loaded — input {keras_model.input_shape}  "
          f"{len(keras_model.outputs)} outputs:")
    for i, out in enumerate(keras_model.outputs):
        print(f"        [{i}] {out.name:30s} shape={out.shape}")
    ...
    print(f"[tflite] wrote {out_path} ({sz:.2f} MB) in {dt:.1f}s")

    # Inspect output tensor metadata
    print()
    print(f"[tflite] inspecting INT8 model metadata...")
    interpreter = tf.lite.Interpreter(model_path=str(out_path))
    interpreter.allocate_tensors()
    print(f"  inputs:")
    for d in interpreter.get_input_details():
        print(f"    {d['name']:30s}  shape={d['shape']}  "
              f"dtype={d['dtype']}  scale={d['quantization'][0]:.6f}  "
              f"zp={d['quantization'][1]}")
    print(f"  outputs:")
    ...
    print()
    print("=" * 70)
    print(f"INT8 TFLite ready: {out_path} ({sz:.2f} MB)")
    print()
    print(f"Next: WSL")
    print(f"  edgetpu_compiler {out_path.name} -o {out_path.parent.name}/")
