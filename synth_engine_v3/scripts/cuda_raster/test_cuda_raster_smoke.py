...
def _win_fonts() -> Path:
    """Resolve Windows fonts dir cross-platform — native Windows uses
    C:\\Windows\\Fonts, WSL Linux sees the same files at /mnt/c/Windows/Fonts."""
    import sys
    if sys.platform.startswith("win"):
        return Path("C:/Windows/Fonts")
    if Path("/mnt/c/Windows/Fonts").is_dir():
        return Path("/mnt/c/Windows/Fonts")
    return Path("/usr/share/fonts/truetype")  # fallback
